...
public class LoanRateIntentHandler implements RequestHandler{

	@Override
    public boolean canHandle(HandlerInput input) {
        return input.matches(intentName("LoanRateIntent"));
    }
    ...
    public Optional<Response> handle(HandlerInput input) {
        ...
        speechText=speechText+" You can look up to the following schemes. ";
        List<LoanRateVO> loanDetailList=db.searchLoandetails(loanTypeId);
    	for(int i=0;i<loanDetailList.size();i++)
    	{
    		speechText=speechText+" Loan "+loanDetailList.get(i).getLoanRate_maxAmount()+" at "+loanDetailList.get(i).getLoanRate_rate()+" % rate for a tenure of "+loanDetailList.get(i).getLoanRate_tenure()+". ";	
    	}
